...
class ModelMapper {
    constructor(type) {
        ...
        this.classMapping = Reflect.getOwnMetadata('classMap', Object.getPrototypeOf(this.target)) || {};
        // console.log(this.target.constructor.name, this.classMapping)
        this.propertyMapping = Reflect.getOwnMetadata('propertyMap', Object.getPrototypeOf(this.target)) || {};
        this.target.constructor.prototype.getClassMapping = () => {
    ...
    serialize(source) {
        if (!source)
            return;
        if (this.classMapping.serialize)
            return this.classMapping.serialize(source);
        const target = {};
        Object.keys(this.propertyMapping).forEach(property => {
            const mapping = this.propertyMapping[property];
            ...
            let value;
            if (typeof mapping.transformer === 'function') {
                value = mapping.transformer(source, this.buildSerializeValue(mapping.type, source, property), target, 'serialize', mapping.source);
            }
            else
    ...
    buildMapValue(type, pathString, source) {
        ...
        if (Array.isArray(type)) {
            return Array.isArray(data)
                ? (0, lodash_1.map)(data, d => this.getMapValue(type[0], d))
                : this.getMapValue(type[0], data);
        }
        else {
    ...
    buildSerializeValue(type, source, property) {
        if (Array.isArray(type)) {
            return (0, lodash_1.map)((0, lodash_1.get)(source, property) || [], (value) => this.getSerializeValue(type[0], value));
        }
        else {
