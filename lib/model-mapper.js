...
class ModelMapper {
    ...
    map(source) {
        ...
        Object.keys(this.propertyMapping).forEach(property => {
            ...
            if (typeof mapping.transformer === 'function') {
                ...
                value = this.buildValue(mapping.type, mapping.source, source);
            if (value !== undefined)
    ...
    serialize(source) {
        ...
        Object.keys(this.propertyMapping).forEach(property => {
            ...
            let value;
            if (typeof mapping.transformer === 'function') {
                value = mapping.transformer(source, this.buildSerializeValue(mapping.type, source, property), target, 'serialize');
            }
            else
